...
import classes from './ListItemHiddenInfo.module.scss'

interface ListItemHiddenInfoProps {
    info: ICargo
}

const ListItemHiddenInfo: FC<ListItemHiddenInfoProps> = ({ info }) => {
    return (
            ...
            <div className={classes.userInfo}>
                <div>
                    <div>
                        ...
                        <StatusList id={info.id} active={info.status} />
                    </div>
                    <div></div>
                    <div>
                        <div className={classes.title}>Дата поступления груза в порт</div>
                        <div className={classes.value}>{info.date ? info.date : '-'}</div>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <div className={classes.value}>{info.manager ? info.manager : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Доставка груза до порта отправления</div>
                        <div className={classes.value}>{info.deliveryToPort ? info.deliveryToPort : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Грузоотправитель</div>
                        <div className={classes.value}>{info.sender ? info.sender : '-'}</div>
                    </div>
                </div>
                <div>
                    <div>
                        <div className={classes.title}>Тип контейнера</div>
                        <div className={classes.value}>{info.cargoType ? info.cargoType : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Масса, тн</div>
                        <div className={classes.value}>{info.weight ? info.weight : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Общий объем, м3</div>
                        <div className={classes.value}>{info.volume ? info.volume : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Длина, метры</div>
                        <div className={classes.value}>{info.length ? info.length : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Высота, метры</div>
                        <div className={classes.value}>{info.height ? info.height : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Ширина, метры</div>
                        <div className={classes.value}>{info.width ? info.width : '-'}</div>
                    </div>
                    <div className={classes.notGrid}>
                        <div className={classes.title}>Описание груза</div>
                        <div className={classes.value}>{info.description ? info.description : '-'}</div>
                    </div>
                </div>
                <div>
                    <div>
                        <div className={classes.title}>ТМЦ</div>
                        <div className={classes.value}>{info.tmts ? info.tmts : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Фрахтовая единица</div>
                        <div className={classes.value}>{info.freightUnit ? info.freightUnit : '-'}</div>
                    </div>
                    <div>
                        <div className={classes.title}>Расположение груза в порту</div>
                        <div className={classes.value}>{info.location ? info.location : '-'}</div>
                    </div>
                </div>
            </div>
            ...
            <div className={`${classes.userInfo} ${classes.userInfoFullWidth}`}>
                <div>
                    ...
                    <div>
                        ...
                        <div className={classes.value}>{info.acceptanceActNumber ? info.acceptanceActNumber : '-'}</div>
                    </div>
...
export default ListItemHiddenInfo
